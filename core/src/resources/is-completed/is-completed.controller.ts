...
export class IsCompletedController {
  ...
  @Get('findAll')
  findAll(@Query('projectId') projectId: number, @Query('kpId') kpId: number, @Query('mqId') mqId: number, @Query('stepId') stepId: number, @Query('noteId') noteId: number) {
    return this.isCompletedService.findAll({ projectId, kpId, mqId, stepId, noteId });
  }
